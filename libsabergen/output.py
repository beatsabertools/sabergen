...
from .core import BeatSaberSong


def __write_json(data, fp):
    "Ensures consistent constraints are used for all JSON file writes."
    return json.dump(
        data, fp, allow_nan=False, skipkeys=False, ensure_ascii=False, check_circular=True, sort_keys=False,
    )
...
def create_song(song: BeatSaberSong):
    ...
    song_info = {
        ...
        "difficultyLevels": [{"difficulty": "Expert+", "difficultyRank": 4, "jsonPath": "expert_plus.json",}],
    }
